...
class Network(torch.nn.Module):
    ...
    def estimate(self, tensorFirst, tensorSecond):
        ...
        tensorFlow = 20.0 * torch.nn.functional.interpolate(
            input=self(tensorPreprocessedFirst, tensorPreprocessedSecond), size=(intHeight, intWidth),
            mode='bilinear', align_corners=False)
...
class Decoder(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        if intLevel < 6:
            self.moduleUpflow = torch.nn.ConvTranspose2d(in_channels=2, out_channels=2, kernel_size=4,
                                                         stride=2, padding=1)
        if intLevel < 6:
